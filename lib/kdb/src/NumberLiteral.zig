...
fn parseSecond(bytes: []const u8, allow_suffix: bool) Result {
    ...
    switch (slice.len) {
        ...
        3, 4, 5 => {
            ...
            const seconds = hour_value * 3600 + minute_value * 60;
            return .{ .second = seconds };
        },
        8, 9 => if (slice[slice.len - 6] == ':' and slice[slice.len - 3] == ':') {
            ...
            if (second_value > 59) return .{ .failure = .overflow };
            const seconds = hour_value * 3600 + minute_value * 60 + second_value;
            return .{ .second = seconds };
        },
        else => {},
    }
    ...
    var seconds: i32 = 0;
    {
        const res = @mulWithOverflow(hour_value, 3600);
        if (res[1] != 0) return .{ .failure = .overflow };
        seconds = res[0];
    }
    {
        const res = @addWithOverflow(seconds, minute_value * 60);
        if (res[1] != 0) return .{ .failure = .overflow };
        seconds = res[0];
    }
    const res = @addWithOverflow(seconds, second_value);
