...
global_error_set: IdentifierSet = .empty,
...
pub const IdentifierTokenContext = struct {
    ...
    pub fn eql(self: @This(), a: Ast.Token.Index, b: Ast.Token.Index, b_index: usize) bool {
        ...
        const b_name = self.tree.tokenSlice(b);
...
pub const ScopeContext = struct {
    ...
    pub fn deinit(context: *ScopeContext) void {
        context.child_scopes_scratch.deinit(context.gpa);
        context.child_declarations_scratch.deinit(context.gpa);
    }

    pub const PushedScope = struct {
    ...
    pub fn startScope(context: *ScopeContext, tag: Scope.Tag, node: Ast.Node.Index, loc: Scope.SmallLoc) error{OutOfMemory}!PushedScope {
        try context.doc_scope.scopes.append(context.gpa, .{
            .data = .{
                .tag = tag,
    ...
    switch (tags[node_idx]) {
        ...
        => {
            ...
            const new_scope = try context.startScope(
                ...
                undefined,
                locToSmallLoc(
                    tree.tokens.items(.loc)[start_token].start,
                    tree.tokens.items(.loc)[last_token].end,
                ),
            );
...
noinline fn walkContainerDecl(
    ...
    const scope = try context.startScope(
        .container,
        .{ .ast_node = node_idx },
        locToSmallLoc(tree.tokens.items(.loc)[first_token].start, tree.tokens.items(.loc)[last_token].end),
    );
...
fn walkBlockNodeKeepOpen(
    ...
    const scope = try context.startScope(
        .block,
        .{ .ast_node = node_idx },
        locToSmallLoc(tree.tokens.items(.loc)[start_token].start, tree.tokens.items(.loc)[last_token].end),
    );
