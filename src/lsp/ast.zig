...
fn fullPtrTypeComponents(tree: Ast, info: Ast.full.PtrType.Components) Ast.full.PtrType {
...
pub fn ptrTypeSimple(tree: Ast, node: Ast.Node.Index) Ast.full.PtrType {
    assert(tree.nodes.items(.tag)[node] == .ptr_type);
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.lhs, Ast.Node.PtrType);
...
pub fn ptrTypeSentinel(tree: Ast, node: Ast.Node.Index) Ast.full.PtrType {
...
pub fn ptrTypeAligned(tree: Ast, node: Ast.Node.Index) Ast.full.PtrType {
...
pub fn ptrTypeBitRange(tree: Ast, node: Ast.Node.Index) Ast.full.PtrType {
    assert(tree.nodes.items(.tag)[node] == .ptr_type_bit_range);
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.lhs, Ast.Node.PtrTypeBitRange);
    return fullPtrTypeComponents(tree, .{
        .main_token = tree.nodes.items(.main_token)[node],
        .align_node = extra.align_node,
        .addrspace_node = extra.addrspace_node,
...
fn fullAsmComponents(tree: Ast, info: Ast.full.Asm.Components) Ast.full.Asm {
    const token_tags = tree.tokens.items(.tag);
    const node_tags = tree.nodes.items(.tag);
    var result: Ast.full.Asm = .{
        .ast = info,
        .volatile_token = null,
        .inputs = &.{},
        .outputs = &.{},
...
pub fn asmSimple(tree: Ast, node: Ast.Node.Index) Ast.full.Asm {
    const data = tree.nodes.items(.data)[node];
...
pub fn asmFull(tree: Ast, node: Ast.Node.Index) Ast.full.Asm {
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.rhs, Ast.Node.Asm);
    return fullAsmComponents(tree, .{
        .asm_token = tree.nodes.items(.main_token)[node],
        .template = data.lhs,
...
fn fullIfComponents(tree: Ast, info: Ast.full.If.Components) Ast.full.If {
    const token_tags = tree.tokens.items(.tag);
    var result: Ast.full.If = .{
        .ast = info,
        .payload_token = null,
        .error_token = null,
        .else_token = 0,
...
pub fn ifFull(tree: Ast, node: Ast.Node.Index) Ast.full.If {
    ...
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.rhs, Ast.Node.If);
...
pub fn ifSimple(tree: Ast, node: Ast.Node.Index) Ast.full.If {
...
fn fullWhileComponents(tree: Ast, info: Ast.full.While.Components) Ast.full.While {
    const token_tags = tree.tokens.items(.tag);
    var result: Ast.full.While = .{
        .ast = info,
        .inline_token = null,
        .label_token = null,
        .payload_token = null,
...
fn fullForComponents(tree: Ast, info: Ast.full.For.Components) Ast.full.For {
    const token_tags = tree.tokens.items(.tag);
    var result: Ast.full.For = .{
        .ast = info,
        .inline_token = null,
        .label_token = null,
...
pub fn whileSimple(tree: Ast, node: Ast.Node.Index) Ast.full.While {
    const data = tree.nodes.items(.data)[node];
...
pub fn whileCont(tree: Ast, node: Ast.Node.Index) Ast.full.While {
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.rhs, Ast.Node.WhileCont);
    return fullWhileComponents(tree, .{
        .while_token = tree.nodes.items(.main_token)[node],
        .cond_expr = data.lhs,
...
pub fn whileFull(tree: Ast, node: Ast.Node.Index) Ast.full.While {
    const data = tree.nodes.items(.data)[node];
    const extra = tree.extraData(data.rhs, Ast.Node.While);
    return fullWhileComponents(tree, .{
        .while_token = tree.nodes.items(.main_token)[node],
        .cond_expr = data.lhs,
        .cont_expr = extra.cont_expr,
...
pub fn forSimple(tree: Ast, node: Ast.Node.Index) Ast.full.For {
    const data = &tree.nodes.items(.data)[node];
    const inputs: *[1]Ast.Node.Index = &data.lhs;
...
pub fn forFull(tree: Ast, node: Ast.Node.Index) Ast.full.For {
    const data = tree.nodes.items(.data)[node];
    const extra = @as(Ast.Node.For, @bitCast(data.rhs));
...
pub fn fullPtrType(tree: Ast, node: Ast.Node.Index) ?Ast.full.PtrType {
...
pub fn fullIf(tree: Ast, node: Ast.Node.Index) ?Ast.full.If {
    return switch (tree.nodes.items(.tag)[node]) {
...
pub fn fullWhile(tree: Ast, node: Ast.Node.Index) ?Ast.full.While {
    return switch (tree.nodes.items(.tag)[node]) {
...
pub fn fullFor(tree: Ast, node: Ast.Node.Index) ?Ast.full.For {
    return switch (tree.nodes.items(.tag)[node]) {
...
pub fn fullAsm(tree: Ast, node: Ast.Node.Index) ?Ast.full.Asm {
    return switch (tree.nodes.items(.tag)[node]) {
    ...
    while (true) switch (tags[n]) {
        ...
        .global_var_decl => {
            ...
            if (datas[n].rhs != 0) {
                ...
                const extra = tree.extraData(datas[n].lhs, Ast.Node.GlobalVarDecl);
        ...
        .local_var_decl => {
            ...
            if (datas[n].rhs != 0) {
                ...
                const extra = tree.extraData(datas[n].lhs, Ast.Node.LocalVarDecl);
        ...
        .call, .async_call => {
            end_offset += 1; // for the rparen
            const params = tree.extraData(datas[n].rhs, Ast.Node.SubRange);
        ...
        .tagged_union_enum_tag => {
            const members = tree.extraData(datas[n].rhs, Ast.Node.SubRange);
        ...
        .@"switch" => {
            const cases = tree.extraData(datas[n].rhs, Ast.Node.SubRange);
        ...
        .@"asm" => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.Asm);
            return extra.rparen + end_offset;
        },
        .array_init,
        .struct_init,
        => {
            const elements = tree.extraData(datas[n].rhs, Ast.Node.SubRange);
        ...
        .container_field => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.ContainerField);
        ...
        .slice => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.Slice);
            assert(extra.end != 0); // should have used slice_open
            end_offset += 1; // rbracket
            n = extra.end;
        },
        .slice_sentinel => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.SliceSentinel);
            if (extra.sentinel != 0) {
        ...
        .fn_proto_multi => {
            const extra = tree.extraData(datas[n].lhs, Ast.Node.SubRange);
        ...
        .fn_proto => {
            const extra = tree.extraData(datas[n].lhs, Ast.Node.FnProto);
            // addrspace, linksection, callconv, align can appear in any order, so we
            // find the last one here.
            // rhs can be zero if no return type is provided
            var max_node: Ast.Node.Index = 0;
        ...
        .while_cont => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.WhileCont);
            assert(extra.then_expr != 0);
            n = extra.then_expr;
        },
        .@"while" => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.While);
            assert(extra.else_expr != 0);
            n = extra.else_expr;
        },
        .@"if" => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.If);
            assert(extra.else_expr != 0);
            n = extra.else_expr;
        },
        .@"for" => {
            const extra = @as(Ast.Node.For, @bitCast(datas[n].rhs));
        ...
        .array_type_sentinel => {
            const extra = tree.extraData(datas[n].rhs, Ast.Node.ArrayTypeSentinel);
            n = extra.elem_type;
        },
    };
}
...
pub fn testDeclNameToken(tree: Ast, test_decl_node: Ast.Node.Index) ?Ast.TokenIndex {
    assert(tree.nodes.items(.tag)[test_decl_node] == .test_decl);
...
pub fn builtinCallParams(tree: Ast, node: Ast.Node.Index, buf: *[2]Ast.Node.Index) ?[]const Ast.Node.Index {
...
pub fn blockStatements(tree: Ast, node: Ast.Node.Index, buf: *[2]Ast.Node.Index) ?[]const Ast.Node.Index {
    const node_data = tree.nodes.items(.data);
    return switch (tree.nodes.items(.tag)[node]) {
...
pub const ErrorSetIterator = struct {
    ...
    pub fn init(tree: Ast, node: Ast.Node.Index) ErrorSetIterator {
        assert(tree.nodes.items(.tag)[node] == .error_set_decl);
        return .{
...
fn iterateChildrenTypeErased(
    ...
    switch (tag) {
        ...
        => {
            ...
            if (lambda.params) |p| {
                for (p.params) |child| try callback(context, tree, child);
            }
...
pub fn nodeChildrenAlloc(allocator: Allocator, tree: Ast, node: Ast.Node.Index) error{OutOfMemory}![]Ast.Node.Index {
...
/// caller owns the returned memory
pub fn nodeChildrenRecursiveAlloc(allocator: Allocator, tree: Ast, node: Ast.Node.Index) error{OutOfMemory}![]Ast.Node.Index {
    const Context = struct {
        children: *std.ArrayList(Ast.Node.Index),
        fn callback(self: @This(), ast: Ast, child_node: Ast.Node.Index) error{OutOfMemory}!void {
            _ = ast;
...
/// caller owns the returned memory.
pub fn nodesOverlappingIndex(allocator: Allocator, tree: Ast, index: usize) error{OutOfMemory}![]Ast.Node.Index {
    assert(index <= tree.source.len);
...
/// caller owns the returned memory
pub fn nodesAtLoc(allocator: Allocator, tree: Ast, loc: offsets.Loc) error{OutOfMemory}![]Ast.Node.Index {
    assert(loc.start <= loc.end and loc.end <= tree.source.len);
...
pub fn smallestEnclosingSubrange(children: []const offsets.Loc, loc: offsets.Loc) ?struct {
    ...
    switch (children.len) {
        ...
        1 => return if (offsets.locInside(loc, children[0])) .{ .start = 0, .len = 1 } else null,
            ...
            //     assert(previous_loc.end <= current_loc.start); // must be sorted
            //     assert(!offsets.locIntersect(previous_loc, current_loc)); // must be non-intersecting
            // }
        },
    }
