...
pub fn deinit(self: *Analyser) void {
    ...
    assert(self.use_trail.count() == 0);
    self.use_trail.deinit(self.gpa);
    self.arena.deinit();
}

pub fn getDocCommentsBeforeToken(allocator: Allocator, tree: Ast, base: Ast.TokenIndex) error{OutOfMemory}!?[]const u8 {
...
pub fn getDocComments(allocator: Allocator, tree: Ast, node: Ast.Node.Index) error{OutOfMemory}!?[]const u8 {
...
pub fn getDocCommentTokenIndex(tokens: []const Ast.Token.Tag, base_token: Ast.TokenIndex) ?Ast.TokenIndex {
    var idx = base_token;
...
pub fn collectDocComments(allocator: Allocator, tree: Ast, doc_comments: Ast.TokenIndex, container_doc: bool) error{OutOfMemory}![]const u8 {
...
pub fn isInstanceCall(
    ...
    assert(container_ty.is_type_val);
...
pub fn hasSelfParam(analyser: *Analyser, func_ty: Type) error{OutOfMemory}!bool {
    ...
    assert(in_container.is_type_val);
...
fn trimCommonIndentation(allocator: Allocator, str: []const u8, preserved_indentation_amount: usize) error{OutOfMemory}![]u8 {
    ...
    assert(output.items.len == output.capacity);
    return output.items;
}
...
pub fn identifierLocFromIndex(tree: Ast, source_index: usize) ?offsets.Loc {
    ...
    switch (tree.tokens.items(.tag)[token]) {
        .identifier => return offsets.tokenToLoc(tree, token),
        else => {},
    }
...
fn resolvePropertyType(analyser: *Analyser, ty: Type, name: []const u8) error{OutOfMemory}!?Type {
    ...
    switch (ty.data) {
        .pointer => |info| switch (info.size) {
            .One => switch (info.elem_ty.data) {
                .array => {
                    assert(!info.elem_ty.is_type_val);
...
fn resolveTypeOfNodeUncached(analyser: *Analyser, node_handle: NodeWithHandle) error{OutOfMemory}!?Type {
    ...
    switch (node_tags[node]) {
        ...
        .identifier => {
            ...
            const child = try analyser.lookupSymbolGlobal(handle, name, token_locs[name_token]) orelse return null;
            return try child.resolveType(analyser);
        },
        .call,
        .call_comma,
        ...
        => {
            ...
            if (std.mem.eql(u8, call_name, "@This")) {
                ...
                return try innermostContainer(handle, token_locs[tree.firstToken(node)]);
            }
...
pub const Type = struct {
    ...
    pub fn fromEither(arena: Allocator, entries: []const TypeWithDescriptor) error{OutOfMemory}!?Type {
        if (entries.len == 0)
            return null;
    ...
    /// Drops duplicates
    pub fn getAllTypesWithHandles(ty: Type, arena: Allocator) ![]const Type {
    ...
    pub fn getAllTypesWithHandlesArrayList(ty: Type, arena: Allocator, all_types: *std.ArrayListUnmanaged(Type)) !void {
        switch (ty.data) {
            .either => |entries| {
                for (entries) |entry| {
    ...
    fn getContainerKind(self: Type) ?Ast.Token.Tag {
    ...
    fn isContainerKind(self: Type, container_kind_tok: Ast.Token.Tag) bool {
    ...
    pub fn docComments(self: Type, allocator: Allocator) error{OutOfMemory}!?[]const u8 {
        if (self.is_type_val) {
            switch (self.data) {
...
pub fn collectImports(allocator: Allocator, tree: Ast) error{OutOfMemory}!std.ArrayListUnmanaged([]const u8) {
    _ = tree; // autofix
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
    ...
    //     if (!std.mem.startsWith(Ast.Token.Tag, tags[i + 1 ..], &.{ .l_paren, .string_literal, .r_paren })) continue;
...
pub fn collectCImportNodes(allocator: Allocator, tree: Ast) error{OutOfMemory}![]Ast.Node.Index {
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
...
pub fn getFieldAccessType(
    ...
    var tokenizer = Tokenizer.init(held_range);
...
fn peek(allocator: Allocator, arr: *std.ArrayListUnmanaged(StackState)) !*StackState {
    if (arr.items.len == 0) {
...
fn tokenLocAppend(prev: offsets.Loc, token: Ast.Token) offsets.Loc {
    return .{
        .start = prev.start,
        .end = token.loc.end,
    };
...
pub fn getPositionContext(
    ...
    var line_loc = if (lookahead)
        offsets.lineLocAtIndex(tree.source, source_index)
    else
        offsets.lineLocUntilIndex(tree.source, source_index);

    if (std.mem.startsWith(u8, std.mem.trimLeft(
        u8,
        offsets.locToSlice(tree.source, line_loc),
        " \t",
    ), "//")) return .comment;
...
pub const DeclWithHandle = struct {
    ...
    pub fn docComments(self: DeclWithHandle, allocator: Allocator) error{OutOfMemory}!?[]const u8 {
        const tree = self.handle.tree;
        return switch (self.decl) {
    ...
    pub fn isPublic(_: DeclWithHandle) bool {
        return true;
    }
...
fn collectUsingnamespaceDeclarationsOfContainer(
    ...
    defer assert(analyser.use_trail.remove(.{ .node = usingnamespace_node.node, .uri = usingnamespace_node.handle.uri }));
...
pub fn collectAllSymbolsAtSourceIndex(
    ...
    assert(source_index <= handle.tree.source.len);
...
fn resolveUse(analyser: *Analyser, uses: []const Ast.Node.Index, symbol: []const u8, handle: *DocumentStore.Handle) error{OutOfMemory}!?DeclWithHandle {
    for (uses) |index| {
        ...
        defer assert(analyser.use_trail.remove(.{ .node = index, .uri = handle.uri }));
...
pub fn lookupSymbolGlobal(
    ...
    const document_scope = try handle.getDocumentScope();
    const scope = innermostScopeAtIndex(document_scope, source_index);
